...
pipclient = OpenAI(api_key=openai_api_key) if openai_api_key else None
# Ensure `client` variable is available for functions below (compatibility)
client = pipclient
...
# SYSTEM_FIELDS used for manual mapping in /map_headers
SYSTEM_FIELDS = [
    "Firstname","Surname","DOB","Gender","Yeargroup","Mastery","Email",
    "Parentname","Parentnumber","Address","Nationality","CountryOfBirth","EnrollmentDate",
    "Conditions","Medication","Allergies","Needs"
...
def current_class():
    if 'user' in session:
        try:
            ...
            connection.close()
            new_period = period_num
            if period_num == 1 or period_num == 5:
                new_period = "Mastery"
            elif period_num > 5:
                new_period -= 2  
            else:
            ...
            return render_template("class.html", students=students_data, class_name=class_name, 
                                   ...
                                   period=new_period, today=today.isoformat(), id=teacher_id,
...
# PURPOSE: Show full post content with details and replies
@app.route('/view_post/<board>/<int:post_id>', methods=['GET'])
def view_post(board, post_id):
    if 'user' in session:
        ...
        # Map boards to their replies tables and FK column name
        replies_table_map = {'general': ('Post_Replies', 'PostID'), 'computing': ('C_Replies', 'CPostID'),
                             'maths': ('M_Replies', 'MPostID'), 'english': ('E_Replies', 'EPostID'),
                             'science': ('S_Replies', 'SPostID'),'history': ('H_Replies', 'HPostID')}
        
        table = table_map.get(board)
        id_column = id_column_map.get(table)
        replies_info = replies_table_map.get(board)
        
        if not table or not replies_info:
        ...
        # Fetch replies
        replies_table, replies_fk = replies_info
        # Include teacher's name with each reply
        cursor.execute(f"SELECT r.*, t.Firstname, t.Surname FROM {replies_table} r LEFT JOIN Teachers t ON r.TeacherID = t.TeacherID WHERE r.{replies_fk}=? ORDER BY r.ReplyID DESC", (post_id,))
        replies = cursor.fetchall()
        # Fetch all teachers for name lookups in templates
        cursor.execute("SELECT * FROM Teachers")
        teachers = cursor.fetchall()
        connection.close()
        
        if post:
            return render_template("view_post.html", post=post, id=id, board=board, replies=replies, teachers=teachers)
        else:
            return redirect(url_for('messages'))
    else:   
        return redirect(url_for('login'))
...
# ROUTE: /add_reply/<board>/<int:post_id> (Add reply to a post)
@app.route('/add_reply/<board>/<int:post_id>', methods=['POST'])
def add_reply(board, post_id):
    if 'user' not in session:
        return redirect(url_for('login'))
    ...
    # Determine replies table and FK column
    replies_table_map = {'general': ('Post_Replies', 'PostID'),'computing': ('C_Replies', 'CPostID'),
                         'maths': ('M_Replies', 'MPostID'),'english': ('E_Replies', 'EPostID'),
                         'science': ('S_Replies', 'SPostID'),'history': ('H_Replies', 'HPostID')}
    ...
    if not replies_info:
        connection.close()
        flash('Invalid board.', 'error')
        return redirect(url_for('messages'))

    replies_table, replies_fk = replies_info

    try:
        cursor.execute(f"INSERT INTO {replies_table} ({replies_fk}, Content, Date, Time, TeacherID) VALUES (?, ?, ?, ?, ?)",
                       (post_id, content, date, time, teacherid))
        connection.commit()
    except Exception as e:
        connection.rollback()
        flash(f'Reply failed: {e}', 'error')
    finally:
        connection.close()
...
# ROUTE: /replies/<board>/<post_id> (Return replies as JSON)
@app.route('/replies/<board>/<int:post_id>', methods=['GET'])
def get_replies(board, post_id):
    if 'user' not in session:
        return jsonify([])
    ...
    replies_table, replies_fk = replies_info
    try:
        cursor.execute(f"SELECT r.ReplyID, r.Content, r.Date, r.Time, r.TeacherID, t.Firstname, t.Surname FROM {replies_table} r LEFT JOIN Teachers t ON r.TeacherID = t.TeacherID WHERE r.{replies_fk}=? ORDER BY r.ReplyID DESC", (post_id,))
        rows = cursor.fetchall()
        data = []
        for row in rows:
            data.append({
                'reply_id': row[0],'content': row[1],
                'date': row[2],'time': row[3],
                'teacher_id': row[4],'firstname': row[5],
                'surname': row[6]
            })
    finally:
        connection.close()
...
# ROUTE: /delete_reply/<board>/<post_id>/<reply_id> (Delete a reply — only owner allowed)
@app.route('/delete_reply/<board>/<int:post_id>/<int:reply_id>', methods=['POST'])
def delete_reply(board, post_id, reply_id):
    if 'user' not in session:
        return redirect(url_for('login'))
    ...
    cursor.execute(f"SELECT TeacherID FROM {replies_table} WHERE ReplyID=?", (reply_id,))
    row = cursor.fetchone()
    if not row:
        connection.close()
    ...
    try:
        cursor.execute(f"DELETE FROM {replies_table} WHERE ReplyID=?", (reply_id,))
        connection.commit()
    except Exception as e:
        connection.rollback()
        flash(f'Delete failed: {e}', 'error')
    ...
    # If AJAX request, return JSON for client-side update
...
def new_post(board):
    if 'user' in session:
        ...
        if request.method == 'POST':
            ...
            content = request.form.get('content').title()
            date = str(datetime.date.today())
            time = datetime.datetime.now().strftime("%H:%M")
            attachments = None
...
def delete_post(board):
    if 'user' in session:
        ...
        # Provide full teachers list so templates can look up any author's name
        teachers = cursor.execute("SELECT * FROM Teachers").fetchall()

        return render_template("delete_post.html", myposts=myposts, id=teacherid, board=board, role=role, teachers=teachers)
    ...
    if 'user' in session:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                flash('Invalid year group selected', 'error')
                return render_template("add_student.html", id=teacherid, subjects=subjects, firstname=teacher[1], surname=teacher[2], email=session['user'], role='T')

            valid_ages = {12: (16, 18), 13: (17, 19)}
            if yeargroup_int not in valid_ages:
                flash('Invalid year group selected', 'error')
                return render_template("add_student.html", id=teacherid, subjects=subjects, firstname=teacher[1], surname=teacher[2], email=session['user'], role='T')
...
def import_students():
    ...
    if request.method == 'POST':
        ...
        try:
            # Read entire upload (handle BOM for Windows Excel exports)
            raw = file.stream.read().decode("utf-8-sig")

            # Attempt to detect delimiter automatically (comma, tab, or semicolon)
            sample = raw[:4096]
            try:
            ...
            if len(rows) == 0:
                flash('No rows found in uploaded file.', 'error')
                return redirect(request.url)

            # Check teacher exists (avoid server error when teacher lookup returns None)
            if not teacher:
                flash('Teacher not found. Please log in again.', 'error')
                return redirect(url_for('login'))

            # Filter out subject columns from mapping/UI (they will be ignored)
            filtered_headers = [h for h in headers if h not in ("FirstSubject","SecondSubject","ThirdSubject","FourthSubject")]

            # VALIDATION: If required headers are present anywhere in the uploaded headers,
            # perform a flexible import (order doesn't matter, extra columns allowed)
            if set(EXPECTED_HEADERS).issubset(set(headers)):
                ...
                # ALGORITHM: Process each student row using header->value mapping
                for i, row in enumerate(rows, start=2):
                    # Create mapping of original headers to row values (handles extra columns)
                    row_map = dict(zip(headers, row))

                    # Ensure required fields exist (use empty string if missing)
                    firstname = row_map.get('Firstname', '').title().strip()
                    surname = row_map.get('Surname', '').title().strip()
                    dob = row_map.get('DOB', '').strip()
                    gender = (row_map.get('Gender', '') or '').strip()[:1].upper()
                    mastery = (row_map.get('Mastery', '') or '').upper().strip()
                    yeargroup = (row_map.get('Yeargroup', '') or '').strip()
                    email = (row_map.get('Email', '') or '').lower().strip()

                    parentname = row_map.get('Parentname', '').title().strip()
                    parentnumber = (row_map.get('Parentnumber', '') or '').strip()
                    address = row_map.get('Address', '').strip()
                    nationality = row_map.get('Nationality', '').title().strip()
                    cob = row_map.get('CountryofBirth', '').title().strip()
                    enrolldate = row_map.get('EnrollmentDate', '').strip()

                    conditions = row_map.get('Conditions', '').title().strip()
                    medication = row_map.get('Medication', '').title().strip()
                    allergies = row_map.get('Allergies', '').title().strip()
                    needs = row_map.get('Needs', '').title().strip()

                    # Basic validation: firstname and surname required
                ...
                # Save filtered headers for mapping UI (subjects removed)
                session['uploaded_headers'] = filtered_headers

                # Store original headers+rows server-side (avoid large cookies)
                import json, uuid, os
                tmp_path = f"/tmp/uploaded_rows_{uuid.uuid4().hex}.json"
                with open(tmp_path, 'w', encoding='utf-8') as f:
                    json.dump({'headers': headers, 'rows': rows}, f)
                session['uploaded_rows_file'] = tmp_path
...
def confirm_mapped_import():
    if 'user' in session:
        ...
        uploaded_rows_file = session.get('uploaded_rows_file')

        # Load rows and original headers from the server-side temp file
        if not uploaded_rows_file or not os.path.exists(uploaded_rows_file):
            return redirect(url_for('import_students'))

        with open(uploaded_rows_file, 'r', encoding='utf-8') as f:
            data_blob = __import__('json').load(f)
        original_headers = data_blob.get('headers', [])
        rows = data_blob.get('rows', [])
        ...
        for row_num, row in enumerate(rows, start=2):
            # Use original headers for correct column alignment
            row_data = dict(zip(original_headers, row))

            data = {field: "" for field in SYSTEM_FIELDS}
            for csv_col, system_col in mapping.items():
                # mapping csv_col refers to the filtered header name shown to user, so values are looked up
        ...
        # cleanup temporary file and session keys
        try:
            os.remove(uploaded_rows_file)
        except Exception:
            pass

        session.pop('uploaded_headers', None)
        session.pop('uploaded_rows_file', None)
